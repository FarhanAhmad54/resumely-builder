        ...
        ats: {
            ...
            render: function (data, settings) {
                ...
                <div class="resume-content resume-ats" style="font-family: Arial, sans-serif; font-size: ${settings.fontSize}pt; padding: 40px;">
                    ...
                    ${data.languages.length > 0 ? `<div class="resume-section"><div style="font-weight: bold; border-bottom: 1px solid #000; margin-bottom: 8px;">LANGUAGES</div><p>${data.languages.map(l => `${escapeHtml(l.name)} (${l.proficiency})`).join(', ')}</p></div>` : ''}
                    ${data.awards.length > 0 ? `<div class="resume-section"><div style="font-weight: bold; border-bottom: 1px solid #000; margin-bottom: 8px;">AWARDS</div>${data.awards.map(a => `<div style="margin-bottom: 8px;"><strong>${escapeHtml(a.title)}</strong>${a.issuer ? ` - ${escapeHtml(a.issuer)}` : ''}${a.date ? ` (${a.date})` : ''}</div>`).join('')}</div>` : ''}
                    ${data.references.length > 0 ? `<div class="resume-section"><div style="font-weight: bold; border-bottom: 1px solid #000; margin-bottom: 8px;">REFERENCES</div>${data.references.map(r => `<div style="margin-bottom: 8px;"><strong>${escapeHtml(r.name)}</strong> - ${escapeHtml(r.position)}${r.company ? ` at ${escapeHtml(r.company)}` : ''}<br>${[r.email, r.phone].filter(Boolean).join(' | ')}</div>`).join('')}</div>` : ''}
                </div>`;
            }
        },
        executive: {
        ...
        tech: {
            ...
            render: function (data, settings) {
                ...
                <div class="resume-content resume-tech" style="--accent: ${settings.accentColor}; font-family: 'Roboto', sans-serif; font-size: ${settings.fontSize}pt; padding: 40px; background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); color: #eee; min-height: 100%;">
                    ...
                    ${renderProjects(data.projects)}
                    ${renderEducation(data.education)}
                    ${data.certifications.length > 0 ? `<div style="margin-bottom: 25px;"><div style="color: ${settings.accentColor}; font-weight: 600; margin-bottom: 12px; text-transform: uppercase; letter-spacing: 1px;">Certifications</div>${data.certifications.map(c => `<div style="margin-bottom: 10px; padding: 12px; background: rgba(255,255,255,0.03); border-radius: 6px;"><strong style="color: #fff;">${escapeHtml(c.name)}</strong><div style="color: #888; font-size: 9pt; margin-top: 4px;">${escapeHtml(c.issuer)}${c.date ? ` • ${c.date}` : ''}</div></div>`).join('')}</div>` : ''}
                    ${data.languages.length > 0 ? `<div style="margin-bottom: 25px;"><div style="color: ${settings.accentColor}; font-weight: 600; margin-bottom: 12px; text-transform: uppercase; letter-spacing: 1px;">Languages</div><div style="display: flex; flex-wrap: wrap; gap: 10px;">${data.languages.map(l => `<span style="padding: 6px 12px; background: rgba(255,255,255,0.1); color: #ddd; border-radius: 4px; font-size: 9pt;">${escapeHtml(l.name)} - ${l.proficiency}</span>`).join('')}</div></div>` : ''}
                    ${data.awards.length > 0 ? `<div style="margin-bottom: 25px;"><div style="color: ${settings.accentColor}; font-weight: 600; margin-bottom: 12px; text-transform: uppercase; letter-spacing: 1px;">Awards</div>${data.awards.map(a => `<div style="margin-bottom: 10px; padding: 12px; background: rgba(255,255,255,0.03); border-radius: 6px;"><strong style="color: #fff;">${escapeHtml(a.title)}</strong>${a.issuer ? `<div style="color: #888; font-size: 9pt; margin-top: 4px;">${escapeHtml(a.issuer)}${a.date ? ` • ${a.date}` : ''}</div>` : ''}</div>`).join('')}</div>` : ''}
                    ${data.references.length > 0 ? `<div style="margin-bottom: 25px;"><div style="color: ${settings.accentColor}; font-weight: 600; margin-bottom: 12px; text-transform: uppercase; letter-spacing: 1px;">References</div><div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;">${data.references.map(r => `<div style="padding: 15px; background: rgba(255,255,255,0.05); border-radius: 8px;"><div style="font-weight: 600; color: #fff;">${escapeHtml(r.name)}</div><div style="font-size: 9pt; color: #aaa;">${escapeHtml(r.position)}${r.company ? ` at ${escapeHtml(r.company)}` : ''}</div><div style="font-size: 9pt; color: #888; margin-top: 6px;">${[r.email, r.phone].filter(Boolean).join(' • ')}</div></div>`).join('')}</div></div>` : ''}
                </div>`;
            }
        },
        elegant: {
        ...
        infographic: {
            ...
            render: function (data, settings) {
                ...
                <div class="resume-content resume-infographic" style="--accent: ${settings.accentColor}; font-family: ${getFontFamily(settings.fontStyle)}; font-size: ${settings.fontSize}pt; padding: 40px; background: #fafafa;">
                    ...
                    ${data.certifications.length > 0 ? `<div style="margin-bottom: 30px; background: white; padding: 25px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);"><div style="font-size: 11pt; font-weight: 600; color: #333; margin-bottom: 15px;">🏅 Certifications</div>${data.certifications.map(c => `<div style="display: flex; align-items: center; gap: 15px; margin-bottom: 12px;"><div style="width: 8px; height: 8px; background: ${settings.accentColor}; border-radius: 50%;"></div><div><span style="font-weight: 600;">${escapeHtml(c.name)}</span> <span style="color: #666;">| ${escapeHtml(c.issuer)}</span></div></div>`).join('')}</div>` : ''}
                    ${data.languages.length > 0 ? `<div style="margin-bottom: 30px; background: white; padding: 25px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);"><div style="font-size: 11pt; font-weight: 600; color: #333; margin-bottom: 15px;">🌐 Languages</div><div style="display: flex; flex-wrap: wrap; gap: 10px;">${data.languages.map(l => `<span style="padding: 8px 15px; background: ${settings.accentColor}; color: white; border-radius: 25px; font-size: 9pt;">${escapeHtml(l.name)} - ${l.proficiency}</span>`).join('')}</div></div>` : ''}
                    ${data.awards.length > 0 ? `<div style="margin-bottom: 30px; background: white; padding: 25px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);"><div style="font-size: 11pt; font-weight: 600; color: #333; margin-bottom: 15px;">🏆 Awards</div>${data.awards.map(a => `<div style="display: flex; align-items: center; gap: 15px; margin-bottom: 12px;"><div style="width: 8px; height: 8px; background: ${settings.accentColor}; border-radius: 50%;"></div><div><span style="font-weight: 600;">${escapeHtml(a.title)}</span>${a.issuer ? ` <span style="color: #666;">| ${escapeHtml(a.issuer)}</span>` : ''}</div></div>`).join('')}</div>` : ''}
                    ${data.references.length > 0 ? `<div style="margin-bottom: 30px; background: white; padding: 25px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);"><div style="font-size: 11pt; font-weight: 600; color: #333; margin-bottom: 15px;">👥 References</div><div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px;">${data.references.map(r => `<div style="padding: 15px; background: #f8f8f8; border-radius: 12px;"><div style="font-weight: 600; color: #333;">${escapeHtml(r.name)}</div><div style="font-size: 9pt; color: #666;">${escapeHtml(r.position)}${r.company ? ` at ${escapeHtml(r.company)}` : ''}</div><div style="font-size: 9pt; color: #888; margin-top: 6px;">${[r.email, r.phone].filter(Boolean).join(' • ')}</div></div>`).join('')}</div></div>` : ''}
                </div>`;
            }
        },
        magazine: {
                ...
                <div class="resume-content resume-magazine" style="--accent: ${settings.accentColor}; font-family: 'Georgia', serif; font-size: ${settings.fontSize}pt; padding: 50px; background: white;">
                    ...
                    <div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 40px;">
                        ...
                        <div style="border-left: 1px solid #eee; padding-left: 30px;">
                            ...
                            ${data.awards.length > 0 ? `<div style="margin-top: 30px;"><div style="font-size: 11pt; text-transform: uppercase; letter-spacing: 3px; color: ${settings.accentColor}; margin-bottom: 15px; font-family: sans-serif; font-weight: 600;">Awards</div>${data.awards.map(a => `<div style="margin-bottom: 10px; font-size: 10pt;"><strong>${escapeHtml(a.title)}</strong>${a.issuer ? `<div style="color: #666;">${escapeHtml(a.issuer)}</div>` : ''}</div>`).join('')}</div>` : ''}
                            ${data.references.length > 0 ? `<div style="margin-top: 30px;"><div style="font-size: 11pt; text-transform: uppercase; letter-spacing: 3px; color: ${settings.accentColor}; margin-bottom: 15px; font-family: sans-serif; font-weight: 600;">References</div>${data.references.map(r => `<div style="margin-bottom: 12px; font-size: 10pt;"><strong>${escapeHtml(r.name)}</strong><div style="color: #666;">${escapeHtml(r.position)}${r.company ? ` at ${escapeHtml(r.company)}` : ''}</div></div>`).join('')}</div>` : ''}
                        </div>
                    </div>
                </div>`;
            }
            ...
            render: function (data, settings) {
                ...
                <div class="resume-content resume-glass" style="--accent: ${settings.accentColor}; font-family: ${getFontFamily(settings.fontStyle)}; font-size: ${settings.fontSize}pt; min-height: 100%; background: linear-gradient(135deg, ${settings.accentColor} 0%, ${adjustColor(settings.accentColor, -60)} 50%, #1a1a2e 100%); padding: 40px;">
                    <div style="background: rgba(255,255,255,0.1); backdrop-filter: blur(10px); border-radius: 20px; padding: 35px; border: 1px solid rgba(255,255,255,0.2); box-shadow: 0 8px 32px rgba(0,0,0,0.3);">
                        ...
                        ${data.projects.length > 0 ? `<div style="margin-bottom: 25px;"><div style="font-size: 11pt; font-weight: 600; color: white; margin-bottom: 15px;">Projects</div><div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px;">${data.projects.map(p => `<div style="padding: 15px; background: rgba(255,255,255,0.08); border-radius: 10px; border: 1px solid rgba(255,255,255,0.1);"><div style="font-weight: 600; color: white; font-size: 10pt;">${escapeHtml(p.name)}</div>${p.technologies ? `<div style="font-size: 9pt; color: rgba(255,255,255,0.6); margin-top: 6px;">${escapeHtml(p.technologies)}</div>` : ''}</div>`).join('')}</div></div>` : ''}
                        ${data.certifications.length > 0 ? `<div style="margin-bottom: 25px;"><div style="font-size: 11pt; font-weight: 600; color: white; margin-bottom: 12px;">Certifications</div>${data.certifications.map(c => `<div style="margin-bottom: 10px; padding: 12px; background: rgba(255,255,255,0.06); border-radius: 8px;"><div style="font-weight: 600; color: white;">${escapeHtml(c.name)}</div><div style="font-size: 9pt; color: rgba(255,255,255,0.7);">${escapeHtml(c.issuer)}${c.date ? ` • ${c.date}` : ''}</div></div>`).join('')}</div>` : ''}
                        ${data.languages.length > 0 ? `<div style="margin-bottom: 25px;"><div style="font-size: 11pt; font-weight: 600; color: white; margin-bottom: 12px;">Languages</div><div style="display: flex; flex-wrap: wrap; gap: 8px;">${data.languages.map(l => `<span style="padding: 6px 14px; background: rgba(255,255,255,0.15); color: white; border-radius: 20px; font-size: 9pt; border: 1px solid rgba(255,255,255,0.2);">${escapeHtml(l.name)} - ${l.proficiency}</span>`).join('')}</div></div>` : ''}
                        ${data.awards.length > 0 ? `<div style="margin-bottom: 25px;"><div style="font-size: 11pt; font-weight: 600; color: white; margin-bottom: 12px;">Awards</div>${data.awards.map(a => `<div style="margin-bottom: 10px; padding: 12px; background: rgba(255,255,255,0.06); border-radius: 8px;"><div style="font-weight: 600; color: white;">${escapeHtml(a.title)}</div>${a.issuer ? `<div style="font-size: 9pt; color: rgba(255,255,255,0.7);">${escapeHtml(a.issuer)}${a.date ? ` • ${a.date}` : ''}</div>` : ''}</div>`).join('')}</div>` : ''}
                        ${data.references.length > 0 ? `<div style="margin-bottom: 25px;"><div style="font-size: 11pt; font-weight: 600; color: white; margin-bottom: 12px;">References</div><div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px;">${data.references.map(r => `<div style="padding: 15px; background: rgba(255,255,255,0.08); border-radius: 10px; border: 1px solid rgba(255,255,255,0.1);"><div style="font-weight: 600; color: white;">${escapeHtml(r.name)}</div><div style="font-size: 9pt; color: rgba(255,255,255,0.7);">${escapeHtml(r.position)}${r.company ? ` at ${escapeHtml(r.company)}` : ''}</div><div style="font-size: 9pt; color: rgba(255,255,255,0.6); margin-top: 6px;">${[r.email, r.phone].filter(Boolean).join(' • ')}</div></div>`).join('')}</div></div>` : ''}
                    </div>
                </div>`;
            }
        }
